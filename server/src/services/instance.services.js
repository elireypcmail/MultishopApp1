...
function generateUniqueInstanceName() {
  const randomString = Math.random().toString(36).substring(2, 8)
  const timestamp = new Date().getTime()
  return `instance_${randomString}_${timestamp}`
}

async function createInstanceForClient(clientId, nombreCliente, instancia) {
  try {
    ...
    if (clienteExistsResult.rows.length === 0) {
