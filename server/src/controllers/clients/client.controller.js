...
controller.getClientesInactivos = async (req, res) => {
  try {
    const queryResult = await pool.query('SELECT * FROM cliente WHERE est_financiero = false')
...
controller.postUser = async (req, res) => {
  ...
  try {
    ...
    const { identificacion, nombre, telefono, dispositivos, per_contacto, clave, instancia } = req.body

    const clienteQuery = `
      INSERT INTO cliente(identificacion, nombre, telefono, per_contacto, clave, instancia)
      VALUES($1, $2, $3, $4, $5, $6)
      RETURNING id`
    const clienteValues = [ identificacion, nombre, telefono, per_contacto, clave, instancia ]
...
controller.loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!dispositivoValido) return res.status(403).json({"message": "No puede ingresar con este dispositivo"})

    if (clave !== dispositivoValido.clave) return res.status(403).send({ message: 'Contraseña incorrecta' }) 

    const token = services.generarToken(userId, true, dispositivos)
